import React, { useState } from "react";
import axios from 'axios';
import Pagination from "./Pagination";
import { Link } from "react-router-dom";
import { Row, Col, Card, Button } from 'react-bootstrap';

function Home() {
    const [data, setData] = useState(null); //this will be our list of users(restaurant)
    const [currentPage, setCurrentPage] = useState(1);
    const [usersPerPage, setUsersPerPage] = useState(12);

    React.useEffect(() => {
        async function fetchData() {
            const res = await axios.get("http://localhost:3001/api")//call db deliveboo
            setData(res.data.result);//with this we'll put all the users in data
        }
        fetchData();
        console.log(data);
    }, [])

    //Get users per page
    const indexOfLastUser = currentPage * usersPerPage;
    const indexOfFirstUser = indexOfLastUser - usersPerPage;

    //change page
    function paginate(pageNumber) {
        return setCurrentPage(pageNumber);
    }

    return (
        <div>
            <h1>Home Page</h1>
            <ul>
                <li>
                    {/* Endpoint to route to Home component */}
                    <Link to="/">Home</Link>
                </li>
                <li>
                    {/* Endpoint to route to About component */}
                    <Link to="/about">About</Link>
                </li>
            </ul>

            <main style={{ width: '75%' }}></main>
            <Row className="row-cols-4">{!data ? "Loading..." : data.slice(indexOfFirstUser, indexOfLastUser).map(user => {
                return <Col key={user.id} className="d-flex justify-content-around mb-3">
                    <Card style={{ width: '83%' }}>
                        <Card.Img style={{ height: '209.95px' }} variant="top" src={user.photo ? user.photo : 'data:image/png;base64,'} />
                        <Card.Body>
                            <Card.Title>{user.name}</Card.Title>
                            <Card.Text>
                                Some quick example text to build on the card title and make up the bulk of
                                the card's content. (Restaurant Description)
                            </Card.Text>
                            <Button variant="primary">
                                <Link to="/show">Show</Link>
                            </Button>
                        </Card.Body>
                    </Card>
                </Col>
            })}</Row>

            {data ? (<Pagination usersPerPage={usersPerPage} totalUsers={data.length} paginate={paginate} />) : "Loading..."}
        </div>
    );
}

export default Home;